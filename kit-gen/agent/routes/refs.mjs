...
/* Tàn dư của một đường đã gỡ: bản 09/09/2026 từng tả ảnh thành chữ và cất bản tả
   ...
   LÀM MẤT NỀN TRONG SUỐT» ở đầu gen.sh), nhưng dự án của người dùng đã chạy bản ấy
   thì vẫn còn mấy file này nằm trong `refs/`. Lọc chúng ra khỏi danh sách ảnh, nếu
   không mỗi tấm ảnh từng được tả sẽ mọc thêm một "tấm ảnh" tên `char-lan.png.desc.txt`
   trong lưới. Xoá chúng thì KHÔNG: đó là dữ liệu trong thư mục của người dùng. */
...
export function register(r) {
  ...
  r.get("/api/projects/:id/refs", async ctx => {
    ...
    for (const name of (await readdir(dir).catch(() => []))) {
      if (name.endsWith(DESC_SUFFIX)) continue         // tàn dư bản cũ, không phải một tấm ảnh
