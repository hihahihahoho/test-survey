...
# KHÁC (so với bản THẬT): không tách alpha thật, chỉ ghi file rỗng để kiểm luồng.
# KHÁC (so với engine-fake/slice.py): NGỦ TRƯỚC KHI CẮT. Đó là toàn bộ lý do fixture này
# tồn tại — trong bản một-nhịp cũ, quãng ngủ này là quãng ẢNH BỊ GIỮ LẠI: `sheet.ready`
# (thứ mang `artifact.path` tới web) chỉ phát sau khi cắt xong. Ca test đo đúng khoảng
# cách giữa `sheet.image` và `sheet.ready` để chứng minh ảnh KHÔNG còn chờ cắt nữa.
import json, os, sys, time
time.sleep(float(os.environ.get("KITGEN_TEST_SLICE_DELAY_MS", "1200")) / 1000.0)
