...
export async function run({ api, wsRoot, agentDir, pid }) {
  ...
  /* ── NỀN ĐỤC KHÔNG CHẶN GÌ CẢ ───────────────────────────────────────────────
     09/2026 từng có một cổng: `gen.sh` trượt tấm đục ⇒ FAIL + đổi tên ảnh thành
     `.rejected.png`, agent giữ job đỏ và không cắt. Chủ sản phẩm chốt 09/09/2026:
     "cái này cứ để cho nó gen tự nhiên nhé, ko block" — nên phép đo nền chỉ còn là
     một ghi chú trong ngoặc vuông trên dòng `OK`, và tấm đục đi hết đường như mọi
     tấm khác. Thứ duy nhất còn nói ra sự thật là `mode: "rgb"` trong manifest, để
     web dán một dòng chú thích cạnh ảnh; quyết định vẽ lại hay xoá là của người dùng. */
  await it("[alpha] tấm nền đục: engine in OK kèm ghi chú ⇒ job xong, vẫn cắt và vẫn đăng", async () => {
    const { api: aA } = await agentWithEngine("engine-alpha")
    const created = await createBasicProject(aA, { name: "Nen duc", firstVariant: { id: "tet", vi: "Tết", bg: "magenta" } })
    const gid = created.json.project.id
    ...
    eq(got.json.status, "done", "không tấm nào hỏng ⇒ lượt chạy sạch")
    const duc = got.json.jobs.find(j => j.sheet === "bg-home")
    eq(duc.status, "ok", "nền đục KHÔNG còn là lý do đánh trượt job")
    eq(duc.diagnosis ?? null, null, "job xong thì không có chẩn đoán nào")
    ok(duc.artifact, "tấm đục vẫn được snapshot như mọi tấm khác")
    eq(got.json.failSummary ?? null, null, "không có câu gộp lỗi nào để mà nói")
    eq(got.json.progress.done, 3, "cả ba tấm đều tính là xong")

    /* Và nó CÓ MẶT trong kho kit — kèm cờ `mode: "rgb"`, thứ duy nhất còn nói cho
       người dùng biết tấm này không có nền trong suốt. */
    const kit = await aA("GET", `/api/projects/${gid}/kit`)
    eq(kit.status, 200, "kit đã cắt")
    const bgFiles = kit.json.files.filter(f => f.sheet === "bg-home")
    ok(bgFiles.length > 0, "tấm đục phải được cắt và nằm trong kho kit")
    ok(bgFiles.every(f => f.mode === "rgb"), `ô cắt ra phải mang cờ rgb: ${JSON.stringify(bgFiles[0])}`)
    const khac = kit.json.files.filter(f => f.sheet && f.sheet !== "bg-home")
    ok(khac.every(f => f.mode === "alpha"), "tấm có alpha thật KHÔNG bị dán oan cờ rgb")
