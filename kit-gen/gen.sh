...
# ║ Ba lượt đo liên tiếp trên cùng một tấm (1254², lưới 2×2 ⇒ ô 627px; banner ô 1  ║
# ║ khai «core aspect 3.9:1»), đo cột có sơn α≥32 ở nửa trên:                      ║
# ║   · r-0040 — dòng ô CHƯA có hộp nào:          x=46..864 ⇒ lấn ô 2 mất 237 px;  ║
# ║   · r-0044 — có «its cell is x=0..627 … stays inside that cell» (11cf121):     ║
# ║                                                x=29..740 ⇒ lấn 113 px;         ║
# ║   · r-0047 — cùng câu ấy, lượt khác:           x=25..690 ⇒ lấn  63 px.         ║
# ║                                                                                ║
# ║ Đọc ra hai điều. ① Hộp bằng số CÓ tác dụng: 237 → 113 → 63 px. ② Model không   ║
# ║ dừng ĐÚNG ở mép hộp — nó tràn vài chục tới trăm px, và phần tràn luôn là TRANG  ║
# ║ TRÍ (nhánh holly, cái nơ ở hai đầu banner). Mà dao cắt (`slice.py`) cắt theo Ô: ║
# ║ phần tràn ấy vừa bị chém cụt vừa dính sang ảnh của ô bên cạnh.                  ║
# ║                                                                                ║
# ║ Nên hộp ta HỨA không còn là ô: nó là ô THỤT VÀO đúng cái lề mà `geometry.py`    ║
# ║ đã định cho chính ô ấy (`cell_margin_ratio` → 0,10, hoặc 0,20 nếu ô có trang    ║
# ║ trí). Model vẫn tràn vài chục px như thường lệ — nhưng nay nó tràn vào phần ô   ║
# ║ CÒN TRỐNG, tức vẫn nằm trong khung dao cắt, chứ không tràn sang hàng xóm. Lề    ║
# ║ 62px trên ô 627 vừa đúng cỡ phần tràn đo được ở r-0047.                        ║
# ║                                                                                ║
# ║ ⚠️ Tỉ lệ lề chỉ được lấy từ `geometry.cell_margin_ratio` / `geometry.cell_inner`:║
# ║ `slice.py` dựng safe zone bằng đúng hai hàm ấy. Gõ tay 0.10 ở đây là dựng bản   ║
# ║ lề thứ hai, và hai bản lề lệch nhau LẶNG LẼ.                                    ║
# ╚════════════════════════════════════════════════════════════════════════════════╝
def inner_box(cell, skel):
    """Hộp THỤT VÀO của một ô → ``(x0, y0, x1, y1)`` trên sheet.

    Căn giữa bằng `//2` (chia lấy nguyên) chứ không `round(…/2)` — đúng phép mà
...
def gutter_px(cell_w, cell_h):
    ...
    Lấy cạnh HẸP HƠN trong hai cạnh: câu trong prompt chỉ có chỗ cho MỘT con số, và
    một lời hứa nhỏ hơn sự thật thì vẫn đúng ở cả hai trục, còn lớn hơn thì sai ở
    một trục. Lề THƯỜNG chứ không phải lề trang trí: đây là dải trống chung cho mọi
    ô của tấm, mà ô có trang trí chỉ chừa RỘNG HƠN chứ không hẹp hơn.
    """
    iw, ih = geometry.cell_inner(cell_w, cell_h)
    return min((cell_w - iw) // 2, (cell_h - ih) // 2)
...
for s in cfg["styles"]:
    for sh in cfg["sheets"]:
        ...
        if profile == "screen":
            ...
            # tấm; hộp toạ độ của từng ô thì ở dòng của ô ấy. Tấm 1 ô không in CỠ Ô:
            # ô ấy chính là khổ ảnh, và «Canvas» đã nói khổ ảnh ở dòng thứ hai của
            # prompt. (Hộp THỤT VÀO ở dòng element thì tấm 1 ô vẫn in — nó là một con
            # số khác khổ ảnh; xem khối «HỘP Ô QUAY LẠI» bên dưới.)
            cell_px = ""
            _cw = _ch = 0
            if cols * rows > 1:
        ...
        if profile == "ui":
            ...
            if cols * rows > 1:
                ...
                geom.append(
                    ...
                    " image edges.")
                # 15/09/2026 — TẤM 1 Ô CŨNG CÓ HỘP, NÊN CŨNG CÓ CÂU XẾP HẠNG.
        ...
        #
        # ╔══ ĐÍNH ẢNH LÀM MẤT NỀN TRONG SUỐT — VÀ TA VẪN ĐÍNH ═══════════════════╗
        # ║ Đo được (chủ sản phẩm, 09/09/2026): cùng một prompt, cùng một model —  ║
        # ║ tấm nào gọi `image_gen` KÈM `referenced_image_paths` thì ảnh về là RGB ║
        # ║ với một cái nền ca-rô do chính model vẽ ra; tấm nào gọi tay không thì  ║
        # ║ về đúng RGBA alpha thật. Tool built-in không có tham số nền nào để xin.║
        # ║                                                                        ║
        # ║ ĐÃ THỬ VÀ ĐÃ BỎ: một bản (09/09/2026) đổi ảnh tham chiếu THÀNH CHỮ     ║
        # ║ bằng một lượt `codex exec` rồi dán đoạn văn ấy vào prompt, để tấm cần  ║
        # ║ alpha không phải đính ảnh nào. Nó giữ được nền trong suốt nhưng đánh   ║
        # ║ mất đúng thứ người ta tải ảnh lên để có: con nhân vật vẽ ra không còn  ║
        # ║ giống ảnh mẫu, vì một đoạn văn thì con thú tròn màu đỏ nào cũng khớp.  ║
        # ║ Chủ sản phẩm chốt: CHẤP NHẬN nền đục giả, chờ codex sửa đầu nguồn,     ║
        # ║ không đi đường tả chữ và cũng không đi đường tách nền sau khi vẽ.      ║
        # ║ ⇒ MỌI loại tấm đính THẲNG mọi ảnh tham chiếu của nó, như dưới đây.     ║
        # ║ Phép đo alpha (`alpha_verdict`) vì thế chỉ còn là GHI CHÚ trong log,   ║
        # ║ không chặn lượt vẽ nào. Ai định "chữa" lại chỗ này: đây là quyết định  ║
        # ║ của chủ sản phẩm, không phải một chỗ bị bỏ quên.                       ║
        # ╚════════════════════════════════════════════════════════════════════════╝
        #
        ...
        # VAI GÌ, cho màn xem trước prompt. Tính hai lần là hai chỗ để lệch.
        att_roles = []
        for role, p in (([("character", sh["ref"])] if sh.get("ref") else [])
                        + ([("pose", sh["poseRef"])] if sh.get("poseRef") else [])
                        ...
                        + ([("layout", sh["layoutRef"])] if sh.get("layoutRef") else [])
                        + [("brand", q) for q in (b["refs"] if use_brand_refs else [])]
                        + [("style", q) for q in (s["inspo"] if use_inspo else [])]):
        ...
        # ── BẢN KÊ CHO MÀN XEM TRƯỚC — `vai<TAB>đường dẫn` ─────────────────────
        # ╔══ VÌ SAO VAI ĐƯỢC ENGINE GHI RA CHỨ KHÔNG SUY LẠI Ở JS ═══════════════╗
        # ║ Màn xem trước prompt phải nói được «tấm ảnh này vào đây với vai gì» —  ║
        # ║ Nhân vật, Dáng, Phong cách, Thương hiệu hay Bố cục. `.att` chỉ có một  ║
        # ║ cột đường dẫn. Suy lại vai ở TypeScript nghĩa là dựng bản sao thứ hai  ║
        # ║ của thứ tự trên, bằng một ngôn ngữ khác, trong một gói cập nhật khác   ║
        # ║ nhịp với engine — đúng loại bản sao trôi khỏi bản gốc trong im lặng.   ║
        # ╚════════════════════════════════════════════════════════════════════════╝
        # Ảnh thương hiệu và ảnh cảm hứng tách hai vai vì người dùng tải chúng lên ở
        # HAI chỗ khác nhau trong bản thiết kế; hỏi «ảnh này ở đâu ra» thì đó mới là
        # câu trả lời. Prompt thì gọi chung cả hai là "brand / inspiration".
