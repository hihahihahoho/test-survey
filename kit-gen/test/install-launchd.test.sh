...
RELEASE="$TEST_ROOT/kitgen-runtime-2.1.3"
OLD_RELEASE="$KITGEN_HOME/releases/2.1.2"
...
if [ "$current" = "$KITGEN_HOME/releases/2.1.2" ] && [ -f "$KITGEN_TEST_STATE/registered" ]; then
  printf '%s\n' '{"ok":true}'
  exit 0
fi
exit 22
...
[ "$status" -ne 0 ] || { echo "expected failed 2.1.3 health check" >&2; exit 1; }
