...
function MascotCard({ item }: { item: LibraryItem }) {
  ...
  const [editOpen, setEditOpen] = React.useState(false);
  const [confirmOpen, setConfirmOpen] = React.useState(false);
  return <><article className="rounded-3 border border-line-subtle bg-surface p-3"><div className="aspect-square overflow-hidden rounded-2 bg-raised">{image?<img src={image} alt="" className="size-full object-contain"/>:<ImagePlus className="m-auto size-6 text-fg-muted"/>}</div><div className="mt-3 flex items-start justify-between gap-2"><div className="min-w-0"><h2 className="truncate text-label text-fg-strong">{item.name}</h2><div className="mt-2 flex min-h-5 flex-wrap gap-1">{item.tags.map(tag=><span key={tag} className="rounded-full bg-raised px-2 py-0.5 text-caption text-fg-muted">{tag}</span>)}</div></div><AssetActions name={item.name} onEdit={()=>setEditOpen(true)} onDelete={()=>setConfirmOpen(true)}/></div></article><EditAssetDialog item={item} open={editOpen} onOpenChange={setEditOpen}/><AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Xoá nhân vật “{item.name}”?</AlertDialogTitle><AlertDialogDescription>Chỉ nhân vật này bị xoá khỏi thư viện. Các project khác không bị xoá.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Huỷ</AlertDialogCancel><AlertDialogAction onClick={()=>remove.mutate(item.id,{onSuccess:()=>toast.success("Đã xoá nhân vật")})}>Xoá nhân vật</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></>;
}
