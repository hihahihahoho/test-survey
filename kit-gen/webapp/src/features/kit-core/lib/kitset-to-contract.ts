 ...
 * └──────────────────────────────────────────────────────────────────────────┘
 *
 * ② VẮNG MẶT Ở NẤC MẶC ĐỊNH, VÀ ĐÓ LÀ THIẾT KẾ. `auto` («Tự động theo vật liệu»)
 * có `en` RỖNG ⇒ không nối gì, dòng ô sạch trơn. Luật của nó — kính/băng/ánh sáng
 * xuyên thấu, kim loại/gỗ/đá đục — đúng với MỌI ô nên `gen.sh` nói nó một lần cho
 * cả tấm, ở section `## Transparency`. Nối nó vào đây là in cùng một luật N lần.
 * Nấc `solid` thì NGƯỢC LẠI: nó có câu, vì nó tồn tại để CÃI luật chung ấy.
 *
