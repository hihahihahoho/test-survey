...
export function buildContract(values: KitFormValues): Contract {
  ...
  return contractSchema.parse({ schemaVersion: 4, variants: [{ id: "phong-cach-1", vi: "Phong cách chính", style: values.stylePrompt, bg: "pure vivid magenta #FF00FF", brand: { mode: "colors", primary: values.primary, secondary: values.secondary }, characters: values.hasCharacter ? [{ id: characterId, vi: values.character.species || "Nhân vật", ref: null }] : [] }], sheets, characterPoses: values.hasCharacter ? Array.from({ length: values.poseCount }, (_, i) => `pose-${i + 1}`) : [] });
}
