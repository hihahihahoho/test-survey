...
describe("composerToContract — kết quả phải QUA ĐƯỢC schema contract", () => {
  ...
  it("nấc «Đục hoàn toàn» thì CÓ câu — nó tồn tại để cãi lại luật chung của tấm", () => {
    /* Không có nấc này thì một ô trông-như-kính mà người dùng muốn ĐẶC sẽ không nói
       ra được điều đó: luật chung sẽ vẽ nó xuyên thấu, và họ không có nút nào để bấm. */
    const spec = resolveElementSpec({ spec: "a glass window pane" }, { glaze: "solid" });
    expect(spec).toContain("fully opaque everywhere, alpha 255");
    expect(spec).toContain("whatever material it may look like");
  });
