...
function readCell(raw: unknown, index: number, presets: PresetBundle): UiCell | null {
   ...
   * còn lại→rỗng) — xem bảng ở `glaze.ts` để biết vì sao gỗ/đá/kim loại rơi về
   * rỗng thay vì được cố giữ.
   *
   * `"glazeId" in raw` chứ không phải `str(...) || fallback`: một bản nháp CÓ khai
   * `glazeId` thì lựa chọn của nó thắng, kể cả khi nó rỗng. Hỏi bằng `||` thì người
  ...
  return {
    ...
    /* CỠ RỖNG LÀ DI SẢN, KHÔNG PHẢI MỘT LỰA CHỌN. Bản nháp lưu trước 07/09/2026
       ...
       GIỐNG `glazeId` ngay bên trên ở chỗ đó — từ 08/09/2026 rỗng của đục nền cũng
       là di sản chứ không phải một lựa chọn, và cũng được vá ngay lúc đọc.
