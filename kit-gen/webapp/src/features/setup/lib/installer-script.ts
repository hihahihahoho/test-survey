/**
 * features/setup/lib/installer-script.ts — nội dung `kit-gen-setup.sh` + SHA256 THẬT.
 *
 * TRUNG THỰC VỀ MỘT GIỚI HẠN: web tĩnh này không có server phát hành file, và tôi
 * KHÔNG được bịa một URL tải agent. Vì vậy script được **sinh ngay trong trình duyệt**
 * và tải bằng Blob; SHA256 được **tính thật** bằng `crypto.subtle.digest` trên đúng
 * bytes user tải về — không phải hằng số chép tay. Nên khi user chạy `shasum -a 256`
 * họ ra đúng con số UI đang hiện. (Nghiệp vụ này lấy từ bản vanilla đã qua QA:
 * `web/js/screens/setup/installer-script.js`, có test `setup-script.test.mjs`.)
 *
 * Script chỉ làm 7 việc (khớp accordion "Script sẽ làm gì" của §3-S0):
 *   1 Node ≥20 · 2 python3 · 3 tạo thư mục làm việc · 4 venv + Pillow/numpy ·
 *   5 kiểm codex CLI (CHỈ `[ -f auth.json ]`, không mở, không đọc) ·
 *   6 đếm image_gen (không sinh ảnh, không tốn quota) · 7 in lệnh chạy.
 *
 * KHÔNG sudo · KHÔNG `curl | bash` · KHÔNG đọc/in/lưu thông tin đăng nhập ·
 * KHÔNG tự tải nhị phân từ URL lạ.
 */

export const SCRIPT_NAME = "kit-gen-setup.sh";

/** 7 việc — hiện trong accordion, ĐÚNG thứ tự trong script. */
export const SEVEN_THINGS: readonly string[] = Object.freeze([
  ...
  "Kiểm tra codex CLI đã cài chưa và ĐÃ đăng nhập chưa — chỉ kiểm tra file có tồn tại hay không, không mở, không đọc nội dung.",
...
/** Bytes CHÍNH XÁC của file sẽ tải về — nguồn của cả Blob lẫn SHA256. */
...
/**
 * SHA256 hex của đúng bytes trên.
 * `crypto.subtle` cần secure context (https hoặc 127.0.0.1). Không có ⇒ trả `null` và
 * UI NÓI THẬT là "chưa tính được chữ ký ở đây", chứ không hiện một con số bịa.
 */
export async function scriptSha256(): Promise<string | null> {
...
export function downloadScript(doc: Document = document): number {
  const bytes = scriptBytes();
  const url = URL.createObjectURL(new Blob([bytes as unknown as BlobPart], { type: "application/x-sh" }));
