...
export function StudioScreen({ projectId = "" }: ScreenProps) {
  ...
  useRegisterCommands(() => [
    { id: "kit.draw", label: "Bộ kit · Tạo ảnh", run: () => setGen(true), disabledReason: gate.readOnly ? gate.reason : null },
    { id: "kit.slice", label: "Bộ kit · Tách ảnh", run: slice, disabledReason: gate.readOnly ? gate.reason : null },
    { id: "kit.copy", label: "Bộ kit · Sao chép danh sách sang Figma", run: copy },
    { id: "kit.settings.style", label: "Bộ kit · Chỉnh phong cách", run: () => setSection("style") },
    { id: "kit.settings.brand", label: "Bộ kit · Chỉnh thương hiệu", run: () => setSection("brand") },
    { id: "kit.settings.ui", label: "Bộ kit · Chỉnh thành phần giao diện", run: () => setSection("ui") },
    { id: "kit.settings.characters", label: "Bộ kit · Chỉnh nhân vật", run: () => setSection("characters") },
  ...
  if (project.error || editor.phase === "error" || !contract) return <div className="p-6"><ErrorState title="Chưa mở được bộ kit" description="Bản thiết kế trên máy vẫn an toàn. Hãy thử lại." detail={devDetails(project.error ?? editor.loadError)} actions={<Button onClick={() => void project.refetch()}>Thử lại</Button>} /></div>;

  return <main className="relative min-h-[calc(100vh-var(--kg-header))] pb-12 pt-8">
    ...
    <header className="kg-page mb-6"><p className="eyebrow">Bộ kit</p><h1 className={DISPLAY}>Quản lý từng <span className={SERIF}>phần</span></h1><p className="mt-3 max-w-2xl text-body text-fg-muted">Chỉnh phong cách chung, thành phần giao diện, nhân vật và ảnh nền trong cùng một nơi.</p></header>
    <div className="kg-page studio-production-grid">
      <nav className="studio-scope-nav" aria-label="Phạm vi bộ kit">
        <span className="eyebrow">Phạm vi</span>
        {([ ["overview","Tổng thể",Palette], ["ui","Giao diện",SlidersHorizontal], ["characters","Nhân vật",Users], ["background","Ảnh nền",ImageIcon] ] as const).map(([id,label,Icon]) => <button key={id} className={scope===id?"active":""} onClick={()=>setScope(id)}><Icon aria-hidden/><span>{label}</span><small>{id==="overview"?jobs.length:contract.sheets.filter(sh=>id==="characters"?sh.id.startsWith("pose-"):id==="background"?(sh.id.startsWith("bg")||(sh.grid.cols===1&&sh.grid.rows===1)):!sh.id.startsWith("pose-")&&!sh.id.startsWith("bg")).length}</small></button>)}
        <div className="studio-nav-note"><strong>Phong cách chung</strong><span>{variant?.style || "Chưa có mô tả"}</span><Button variant="ghost" size="sm" onClick={()=>setSection("style")}><Settings2/>Chỉnh phong cách</Button></div>
      </nav>
      <section className="studio-stage">
        {scope === "overview" && <><Section icon={Palette} title="Phong cách chung" description="Mọi phần trong bộ kit dùng chung phong cách này, trừ khi bạn chỉnh riêng." onSettings={()=>setSection("style")}><StyleBlock variant={variant} actions={actions}/></Section><Section icon={Paintbrush} title="Thương hiệu" description="Màu chủ đạo được áp dụng xuyên suốt bộ kit." onSettings={()=>setSection("brand")}><BrandBlock variant={variant}/></Section></>}
        {scope === "ui" && <Section icon={SlidersHorizontal} title="Thành phần giao diện" description="Xem khung bố cục, ảnh đã tạo và ảnh đã tách theo từng tấm. Chọn một món sẽ chọn cả tấm chứa món đó để tạo lại." onSettings={()=>setSection("ui")}><UIBlock projectId={projectId} variantId={variant?.id ?? ""} contract={contract} picked={picked} setPicked={setPicked} onAdd={()=>setLibrary(true)} views={sheetViews} setViews={setSheetViews} projectState={editor.projectState.jobs}/></Section>}
        {scope === "characters" && <Section icon={Users} title="Nhân vật" description="Quản lý nhân vật, các dáng và những phiên bản ảnh đã tạo." onSettings={()=>setSection("characters")}><CharacterBlock variant={variant} actions={actions}/></Section>}
        {scope === "background" && <Section icon={ImageIcon} title="Ảnh nền" description="Quản lý ảnh nền riêng để có thể tạo lại mà không ảnh hưởng phần giao diện." onSettings={()=>setSection("ui")}><UIBlock projectId={projectId} variantId={variant?.id ?? ""} contract={{...contract,sheets:contract.sheets.filter(sh=>sh.id.startsWith("bg")||(sh.grid.cols===1&&sh.grid.rows===1))}} picked={picked} setPicked={setPicked} onAdd={()=>setLibrary(true)} views={sheetViews} setViews={setSheetViews} projectState={editor.projectState.jobs}/></Section>}
      </section>
      <aside className="studio-inspector"><span className="eyebrow">Đang chọn</span><h2>{scope === "overview"?"Toàn bộ":scope === "ui"?"Giao diện":scope === "characters"?"Nhân vật":"Ảnh nền"}</h2><p>{selectedJobs.length} tấm sẽ được tạo</p><div className="studio-inspector-actions"><Button className={CTA} disabled={gate.readOnly||selectedJobs.length===0} onClick={()=>{setGenerateAll(false);setGen(true)}}><Sparkles/>Tạo phần này</Button><Button variant="secondary" disabled={gate.readOnly} onClick={()=>{setGenerateAll(true);setGen(true)}}>Tạo toàn bộ</Button></div><div className="studio-version-note"><strong>Lịch sử theo từng tấm</strong><span>Mỗi tấm giữ tối đa ba phiên bản gần nhất để bạn có thể quay lại khi cần.</span></div></aside>
    </div>
    <div className="kg-page mt-5 flex justify-center"><FloatingToolbar aria-label="Hành động trên bộ kit" left={<Button size="sm" className={CTA} disabled={gate.readOnly} onClick={() => setGen(true)}><Sparkles aria-hidden />Tạo ảnh</Button>} center={<Button variant="ghost" size="sm" disabled={gate.readOnly} onClick={slice}><Scissors aria-hidden />Tách ảnh</Button>} right={<><Button variant="ghost" size="sm" onClick={copy}><Copy aria-hidden />Sao chép sang Figma</Button><span className="hidden px-2 text-caption text-fg-muted sm:inline">{editor.api.dirty ? "Đang tự lưu…" : "Đã tự lưu"}</span></>} /></div>
    <DrawDialog open={gen} onOpenChange={(v)=>{setGen(v);if(!v)setGenerateAll(false)}} count={selectedJobs.length} blocked={doctor.data?.imageGen?.available === false} pending={start.isPending} onDraw={draw} />
...
function SettingsDrawer({ section, onClose, contract, actions, api }: { section: Section | null; onClose: () => void; contract: Contract; actions: ReturnType<typeof useDesignActions>; api: ReturnType<typeof useDesignEditor>["api"] }) {
  ...
  const title = section === "style" ? "Phong cách" : section === "brand" ? "Thương hiệu" : section === "ui" ? "Thành phần giao diện" : "Nhân vật";
  return <Sheet open={section !== null} onOpenChange={(open) => !open && onClose()}><SheetContent side="right"><SheetHeader><SheetTitle>{title}</SheetTitle><SheetDescription>Thay đổi được lưu tự động vào dự án.</SheetDescription></SheetHeader><SheetBody>{section === "style" && variant && <div className="flex flex-col gap-4"><Input aria-label="Tên phong cách" value={variant.vi} onChange={(e) => actions.patchVariant(variant.id, { vi: e.target.value })} placeholder="Tên phong cách" /><Textarea aria-label="Mô tả phong cách" rows={6} value={variant.style} onChange={(e) => actions.patchVariant(variant.id, { style: e.target.value })} placeholder="Mô tả cho máy…" /></div>}{section === "brand" && variant && <div className="flex flex-col gap-4"><label className="text-label text-fg-strong">Màu chính<Input aria-label="Màu chính" type="color" value={variant.brand?.primary ?? ""} onChange={(e) => actions.patchBrand(variant.id, { primary: e.target.value })} /></label><label className="text-label text-fg-strong">Màu phụ<Input aria-label="Màu phụ" type="color" value={variant.brand?.secondary ?? ""} onChange={(e) => actions.patchBrand(variant.id, { secondary: e.target.value })} /></label></div>}{section === "ui" && <div className="flex flex-col gap-4"><p className="text-body text-fg-muted">Thêm thành phần từ thư viện hoặc tạo một tấm trống. Có thể hoàn tác thay đổi bằng ⌘Z.</p><Button variant="secondary" onClick={() => actions.addSheet("blank")} disabled={api.readOnly}>Thêm tấm trống</Button></div>}{section === "characters" && <CharacterBlock variant={variant} actions={actions} />}</SheetBody></SheetContent></Sheet>;
}

function DrawDialog({ open, onOpenChange, count, blocked, pending, onDraw }: { open: boolean; onOpenChange: (open: boolean) => void; count: number; blocked: boolean; pending: boolean; onDraw: () => void }) { return <Dialog open={open} onOpenChange={(value) => !pending && onOpenChange(value)}><DialogContent><DialogHeader><DialogTitle>Tạo {count} tấm ảnh?</DialogTitle><DialogDescription>Máy sẽ tạo lại {count} tấm và giữ phiên bản hiện tại trong lịch sử.</DialogDescription></DialogHeader><div className="flex flex-col gap-3"><p className="text-body text-fg-muted">Bạn có thể tiếp tục chỉnh bộ kit sau khi bắt đầu.</p>{blocked && <div role="alert" className="rounded-3 border border-warn/60 bg-warn/10 p-3 text-body text-fg">Dịch vụ tạo ảnh chưa sẵn sàng. Mở Cài đặt để kiểm tra kết nối.</div>}</div><DialogFooter><Button variant="secondary" disabled={pending} onClick={() => onOpenChange(false)}>Huỷ</Button><Button variant="primary" loading={pending} disabled={blocked || count === 0} onClick={onDraw}><Sparkles aria-hidden />Bắt đầu tạo</Button></DialogFooter></DialogContent></Dialog>; }
