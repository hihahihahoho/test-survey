import { useStartRun } from "./use-runs";
import { contractJobs, type Contract } from "@/lib/types/contract";

/**
 * lib/hooks/use-generate-run.ts — CỬA DUY NHẤT TIÊU QUOTA của web.
 *
 * ══ VÌ SAO NÓ Ở `lib/hooks/` CHỨ KHÔNG Ở TRONG MÀN SOẠN ═══════════════════
 * Hook này từng nằm ở `features/runs/lib/`; màn theo dõi lượt chạy đã bị xoá cùng
 * đợt IA prompt-first, và nơi gọi duy nhất còn lại là `prompt-canvas/lib/gen-queue`.
 * Dời thẳng vào `features/prompt-canvas/` thì `scripts/check-no-gen.mjs` ĐỎ, và nó
 * đỏ ĐÚNG: `prompt-canvas` là vùng cấm của cổng chặn tiêu tiền — chỗ một nút "Vẽ"
 * dễ mọc ra nhất. Cổng ấy chỉ có nghĩa khi cửa tiêu quota nằm NGOÀI vùng nó canh, ở
 * một file được đọc kỹ, cạnh chính `useStartRun` mà nó bọc.
 *
 * UI phải xác nhận quota TRƯỚC khi gọi hook này.
 */
