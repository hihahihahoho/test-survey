...
/* ══ VENDOR FOOTER — DÒNG DUY NHẤT ĐƯỢC THÊM, XEM README.md ═════════════════
   Mọi byte PHÍA TRÊN là bản sao nguyên vẹn của
   `kit-gen/figma-export/figma-h2d.global.js`
   (49115 byte · sha256 735886cc915e8b0be3c366c75ece67c736fe2e227704304a4d4c4c28f00aec01).
   Bundle gốc là IIFE gán vào `var figmaH2D` — trong classic script đó là biến
   toàn cục, nhưng khi Vite nạp file này như một ES module thì `var` chỉ nằm
   trong phạm vi module ⇒ không ai với tới được. Một dòng `export` là cách nạp
   RẺ NHẤT mà không phải `new Function(...)` (eval) hay chèn <script> lúc chạy.
   `__tests__/vendor-integrity.test.ts` băm lại phần phía trên để chứng minh
   không có byte nào của encoder bị sửa. */
